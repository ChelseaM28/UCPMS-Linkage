...
def research_area_similarity(data):
    ...
    '''for i, (ucmps_field, scopus_field) in enumerate(data):
        if i >= 10:
            break
        scopus_field = scopus_field.replace('MED:', '').replace('|', ' ').strip()
        ucmps_field = ucmps_field.replace('(all),', '').replace('(miscellaneous),', '').strip()
        tokens_u = ucmps_field.split()[0:2]
        tokens_s = scopus_field.split()[0:2]
        print(f"--- Pair {i} ---")
        print(f"  UCPMS raw: '{ucmps_field}' → tokens: {tokens_u}")
        print(f"  Scopus raw: '{scopus_field}' → tokens: {tokens_s}")
        print(f"  UCPMS synsets: {wn.synsets(tokens_u[0]) if tokens_u else 'empty'}")
        print(f"  Scopus synsets: {wn.synsets(tokens_s[0]) if tokens_s else 'empty'}")
'''
